...
func Replaceresourcev1alpha3devicetaintruleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapiresourcev1alpha3DeviceTaintRule
...
func CreateReplaceresourcev1alpha3devicetaintruleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_resource.k8s.io_v1alpha3_devicetaintrules_name",
		mcp.WithDescription("replace the specified DeviceTaintRule"),
		...
		mcp.WithObject("spec", mcp.Required(), mcp.Description("Input parameter: DeviceTaintRuleSpec specifies the selector and one taint.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Replaceresourcev1alpha3devicetaintruleHandler(cfg),
	}
}
