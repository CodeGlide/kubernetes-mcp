...
func Replacestoragev1namespacedcsistoragecapacityHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplacestoragev1namespacedcsistoragecapacityTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_storage.k8s.io_v1_namespaces_namespace_csistoragecapacities_name",
		mcp.WithDescription("replace the specified CSIStorageCapacity"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacestoragev1namespacedcsistoragecapacityHandler(cfg),
	}
}
