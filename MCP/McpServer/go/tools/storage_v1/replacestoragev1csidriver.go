...
func Replacestoragev1csidriverHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapistoragev1CSIDriver
...
func CreateReplacestoragev1csidriverTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_storage.k8s.io_v1_csidrivers_name",
		mcp.WithDescription("replace the specified CSIDriver"),
		...
		mcp.WithObject("spec", mcp.Required(), mcp.Description("Input parameter: CSIDriverSpec is the specification of a CSIDriver.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacestoragev1csidriverHandler(cfg),
	}
}
