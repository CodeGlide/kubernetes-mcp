...
func Replacestoragev1volumeattachmentstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapistoragev1VolumeAttachment
		...
		url := fmt.Sprintf("%s/apis/storage.k8s.io/v1/volumeattachments/%s/status%s", cfg.BaseURL, queryString)
...
func CreateReplacestoragev1volumeattachmentstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_storage.k8s.io_v1_volumeattachments_name_status",
		mcp.WithDescription("replace status of the specified VolumeAttachment"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacestoragev1volumeattachmentstatusHandler(cfg),
	}
}
