...
func Replaceresourcev1beta2namespacedresourceclaimstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapiresourcev1beta2ResourceClaim
		...
		url := fmt.Sprintf("%s/apis/resource.k8s.io/v1beta2/namespaces/%s/resourceclaims/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapiresourcev1beta2ResourceClaim
...
func CreateReplaceresourcev1beta2namespacedresourceclaimstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_resource.k8s.io_v1beta2_namespaces_namespace_resourceclaims_name_status",
		mcp.WithDescription("replace status of the specified ResourceClaim"),
		...
		mcp.WithObject("spec", mcp.Required(), mcp.Description("Input parameter: ResourceClaimSpec defines what is being requested in a ResourceClaim and how to configure it.")),
		mcp.WithObject("status", mcp.Description("Input parameter: ResourceClaimStatus tracks whether the resource has been allocated and what the result of that was.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceresourcev1beta2namespacedresourceclaimstatusHandler(cfg),
	}
}
