...
func Replaceresourcev1beta2namespacedresourceclaimHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplaceresourcev1beta2namespacedresourceclaimTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_resource.k8s.io_v1beta2_namespaces_namespace_resourceclaims_name",
		mcp.WithDescription("replace the specified ResourceClaim"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceresourcev1beta2namespacedresourceclaimHandler(cfg),
	}
}
