...
func CreateReplacecorev1namespacedconfigmapTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_v1_namespaces_namespace_configmaps_name",
		...
		mcp.WithBoolean("immutable", mcp.Description("Input parameter: Immutable, if set to true, ensures that data stored in the ConfigMap cannot be updated (only object metadata can be modified). If not set to true, the field can be modified at any time. Defaulted to nil.")),
