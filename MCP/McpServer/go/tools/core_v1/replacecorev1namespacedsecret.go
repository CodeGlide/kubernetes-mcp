...
func CreateReplacecorev1namespacedsecretTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_v1_namespaces_namespace_secrets_name",
		...
		mcp.WithBoolean("immutable", mcp.Description("Input parameter: Immutable, if set to true, ensures that data stored in the Secret cannot be updated (only object metadata can be modified). If not set to true, the field can be modified at any time. Defaulted to nil.")),
		...
		mcp.WithObject("stringData", mcp.Description("Input parameter: stringData allows specifying non-binary secret data in string form. It is provided as a write-only input field for convenience. All keys and values are merged into the data field on write, overwriting any existing values. The stringData field is never output when reading from the API.")),
		mcp.WithString("type", mcp.Description("Input parameter: Used to facilitate programmatic handling of secret data. More info: https://kubernetes.io/docs/concepts/configuration/secret/#secret-types")),
