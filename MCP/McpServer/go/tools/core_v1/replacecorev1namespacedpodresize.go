...
func Replacecorev1namespacedpodresizeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/pods/%s/resize%s", cfg.BaseURL, queryString)
...
func CreateReplacecorev1namespacedpodresizeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_v1_namespaces_namespace_pods_name_resize",
		mcp.WithDescription("replace resize of the specified Pod"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacecorev1namespacedpodresizeHandler(cfg),
	}
}
