...
func Replacecorev1namespacedpodephemeralcontainersHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/api/v1/namespaces/%s/pods/%s/ephemeralcontainers%s", cfg.BaseURL, queryString)
...
func CreateReplacecorev1namespacedpodephemeralcontainersTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_api_v1_namespaces_namespace_pods_name_ephemeralcontainers",
		mcp.WithDescription("replace ephemeralcontainers of the specified Pod"),
		...
		mcp.WithObject("status", mcp.Description("Input parameter: PodStatus represents information about the status of a pod. Status may trail the actual state of a system, especially if the node that hosts the pod cannot contact the control plane.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacecorev1namespacedpodephemeralcontainersHandler(cfg),
	}
}
