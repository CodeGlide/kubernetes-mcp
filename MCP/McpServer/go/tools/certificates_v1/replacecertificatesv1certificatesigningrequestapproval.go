...
func Replacecertificatesv1certificatesigningrequestapprovalHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapicertificatesv1CertificateSigningRequest
		...
		url := fmt.Sprintf("%s/apis/certificates.k8s.io/v1/certificatesigningrequests/%s/approval%s", cfg.BaseURL, queryString)
...
func CreateReplacecertificatesv1certificatesigningrequestapprovalTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_certificates.k8s.io_v1_certificatesigningrequests_name_approval",
		...
		mcp.WithObject("spec", mcp.Required(), mcp.Description("Input parameter: CertificateSigningRequestSpec contains the certificate request.")),
		mcp.WithObject("status", mcp.Description("Input parameter: CertificateSigningRequestStatus contains conditions used to indicate approved/denied/failed status of the request, and the issued certificate.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacecertificatesv1certificatesigningrequestapprovalHandler(cfg),
	}
}
