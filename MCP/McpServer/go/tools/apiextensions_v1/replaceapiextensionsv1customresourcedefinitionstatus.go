...
func Replaceapiextensionsv1customresourcedefinitionstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/apiextensions.k8s.io/v1/customresourcedefinitions/%s/status%s", cfg.BaseURL, queryString)
...
func CreateReplaceapiextensionsv1customresourcedefinitionstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_apiextensions.k8s.io_v1_customresourcedefinitions_name_status",
		...
		mcp.WithObject("spec", mcp.Required(), mcp.Description("Input parameter: CustomResourceDefinitionSpec describes how a user wants their resource to appear")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceapiextensionsv1customresourcedefinitionstatusHandler(cfg),
	}
}
