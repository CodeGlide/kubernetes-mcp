...
func Replaceschedulingv1priorityclassHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapischedulingv1PriorityClass
		...
		url := fmt.Sprintf("%s/apis/scheduling.k8s.io/v1/priorityclasses/%s%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapischedulingv1PriorityClass
...
func CreateReplaceschedulingv1priorityclassTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_scheduling.k8s.io_v1_priorityclasses_name",
		...
		mcp.WithString("preemptionPolicy", mcp.Description("Input parameter: preemptionPolicy is the Policy for preempting pods with lower priority. One of Never, PreemptLowerPriority. Defaults to PreemptLowerPriority if unset.")),
		mcp.WithNumber("value", mcp.Required(), mcp.Description("Input parameter: value represents the integer value of this priority class. This is the actual priority that pods receive when they have the name of this class in their pod spec.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceschedulingv1priorityclassHandler(cfg),
	}
}
