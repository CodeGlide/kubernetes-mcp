...
func Replaceflowcontrolapiserverv1flowschemastatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapiflowcontrolv1FlowSchema
		...
		url := fmt.Sprintf("%s/apis/flowcontrol.apiserver.k8s.io/v1/flowschemas/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapiflowcontrolv1FlowSchema
...
func CreateReplaceflowcontrolapiserverv1flowschemastatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_flowcontrol.apiserver.k8s.io_v1_flowschemas_name_status",
		mcp.WithDescription("replace status of the specified FlowSchema"),
		...
		mcp.WithObject("spec", mcp.Description("Input parameter: FlowSchemaSpec describes how the FlowSchema's specification looks like.")),
		mcp.WithObject("status", mcp.Description("Input parameter: FlowSchemaStatus represents the current state of a FlowSchema.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceflowcontrolapiserverv1flowschemastatusHandler(cfg),
	}
}
