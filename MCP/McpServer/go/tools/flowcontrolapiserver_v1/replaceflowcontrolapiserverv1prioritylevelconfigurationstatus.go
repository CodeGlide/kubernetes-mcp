...
func Replaceflowcontrolapiserverv1prioritylevelconfigurationstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapiflowcontrolv1PriorityLevelConfiguration
		...
		url := fmt.Sprintf("%s/apis/flowcontrol.apiserver.k8s.io/v1/prioritylevelconfigurations/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapiflowcontrolv1PriorityLevelConfiguration
...
func CreateReplaceflowcontrolapiserverv1prioritylevelconfigurationstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_flowcontrol.apiserver.k8s.io_v1_prioritylevelconfigurations_name_status",
		mcp.WithDescription("replace status of the specified PriorityLevelConfiguration"),
		...
		mcp.WithObject("spec", mcp.Description("Input parameter: PriorityLevelConfigurationSpec specifies the configuration of a priority level.")),
		mcp.WithObject("status", mcp.Description("Input parameter: PriorityLevelConfigurationStatus represents the current state of a \"request-priority\".")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Replaceflowcontrolapiserverv1prioritylevelconfigurationstatusHandler(cfg),
	}
}
