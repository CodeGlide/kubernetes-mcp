...
func Replaceadmissionregistrationv1validatingadmissionpolicystatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapiadmissionregistrationv1ValidatingAdmissionPolicy
		...
		url := fmt.Sprintf("%s/apis/admissionregistration.k8s.io/v1/validatingadmissionpolicies/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapiadmissionregistrationv1ValidatingAdmissionPolicy
...
func CreateReplaceadmissionregistrationv1validatingadmissionpolicystatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_admissionregistration.k8s.io_v1_validatingadmissionpolicies_name_status",
		mcp.WithDescription("replace status of the specified ValidatingAdmissionPolicy"),
		...
		mcp.WithObject("spec", mcp.Description("Input parameter: ValidatingAdmissionPolicySpec is the specification of the desired behavior of the AdmissionPolicy.")),
		mcp.WithObject("status", mcp.Description("Input parameter: ValidatingAdmissionPolicyStatus represents the status of an admission validation policy.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceadmissionregistrationv1validatingadmissionpolicystatusHandler(cfg),
	}
}
