...
func Replacenetworkingv1beta1servicecidrstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapinetworkingv1beta1ServiceCIDR
		...
		url := fmt.Sprintf("%s/apis/networking.k8s.io/v1beta1/servicecidrs/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapinetworkingv1beta1ServiceCIDR
...
func CreateReplacenetworkingv1beta1servicecidrstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_networking.k8s.io_v1beta1_servicecidrs_name_status",
		mcp.WithDescription("replace status of the specified ServiceCIDR"),
		...
		mcp.WithObject("spec", mcp.Description("Input parameter: ServiceCIDRSpec define the CIDRs the user wants to use for allocating ClusterIPs for Services.")),
		mcp.WithObject("status", mcp.Description("Input parameter: ServiceCIDRStatus describes the current state of the ServiceCIDR.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacenetworkingv1beta1servicecidrstatusHandler(cfg),
	}
}
