...
func Replaceinternalapiserverv1alpha1storageversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/internal.apiserver.k8s.io/v1alpha1/storageversions/%s%s", cfg.BaseURL, queryString)
...
func CreateReplaceinternalapiserverv1alpha1storageversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_internal.apiserver.k8s.io_v1alpha1_storageversions_name",
		mcp.WithDescription("replace the specified StorageVersion"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replaceinternalapiserverv1alpha1storageversionHandler(cfg),
	}
}
