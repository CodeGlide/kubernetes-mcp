...
func Replaceinternalapiserverv1alpha1storageversionstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapiapiserverinternalv1alpha1StorageVersion
		...
		url := fmt.Sprintf("%s/apis/internal.apiserver.k8s.io/v1alpha1/storageversions/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapiapiserverinternalv1alpha1StorageVersion
...
func CreateReplaceinternalapiserverv1alpha1storageversionstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_internal.apiserver.k8s.io_v1alpha1_storageversions_name_status",
		mcp.WithDescription("replace status of the specified StorageVersion"),
		...
		mcp.WithObject("spec", mcp.Required(), mcp.Description("Input parameter: StorageVersionSpec is an empty spec.")),
		mcp.WithObject("status", mcp.Required(), mcp.Description("Input parameter: API server instances report the versions they can decode and the version they encode objects to when persisting objects in the backend.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Replaceinternalapiserverv1alpha1storageversionstatusHandler(cfg),
	}
}
