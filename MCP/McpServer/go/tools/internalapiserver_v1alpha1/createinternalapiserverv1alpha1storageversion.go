...
func Createinternalapiserverv1alpha1storageversionHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCreateinternalapiserverv1alpha1storageversionTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apis_internal.apiserver.k8s.io_v1alpha1_storageversions",
		mcp.WithDescription("create a StorageVersion"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Createinternalapiserverv1alpha1storageversionHandler(cfg),
	}
}
