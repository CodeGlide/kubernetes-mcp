...
func Replacenodev1runtimeclassHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapinodev1RuntimeClass
		...
		url := fmt.Sprintf("%s/apis/node.k8s.io/v1/runtimeclasses/%s%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapinodev1RuntimeClass
...
func CreateReplacenodev1runtimeclassTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_node.k8s.io_v1_runtimeclasses_name",
		mcp.WithDescription("replace the specified RuntimeClass"),
		...
		mcp.WithString("handler", mcp.Required(), mcp.Description("Input parameter: handler specifies the underlying runtime and configuration that the CRI implementation will use to handle pods of this class. The possible values are specific to the node & CRI configuration.  It is assumed that all handlers are available on every node, and handlers of the same name are equivalent on every node. For example, a handler called \"runc\" might specify that the runc OCI runtime (using native Linux containers) will be used to run the containers in a pod. The Handler must be lowercase, conform to the DNS Label (RFC 1123) requirements, and is immutable.")),
		...
		mcp.WithObject("overhead", mcp.Description("Input parameter: Overhead structure represents the resource overhead associated with running a pod.")),
		mcp.WithObject("scheduling", mcp.Description("Input parameter: Scheduling specifies the scheduling constraints for nodes supporting a RuntimeClass.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Replacenodev1runtimeclassHandler(cfg),
	}
}
