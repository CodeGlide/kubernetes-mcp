...
func Replacestoragemigrationv1alpha1storageversionmigrationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplacestoragemigrationv1alpha1storageversionmigrationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_storagemigration.k8s.io_v1alpha1_storageversionmigrations_name",
		mcp.WithDescription("replace the specified StorageVersionMigration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    Replacestoragemigrationv1alpha1storageversionmigrationHandler(cfg),
	}
}
