...
func Replacestoragemigrationv1alpha1storageversionmigrationstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.Iok8sapistoragemigrationv1alpha1StorageVersionMigration
		...
		url := fmt.Sprintf("%s/apis/storagemigration.k8s.io/v1alpha1/storageversionmigrations/%s/status%s", cfg.BaseURL, queryString)
		...
		var result models.Iok8sapistoragemigrationv1alpha1StorageVersionMigration
...
func CreateReplacestoragemigrationv1alpha1storageversionmigrationstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_storagemigration.k8s.io_v1alpha1_storageversionmigrations_name_status",
		mcp.WithDescription("replace status of the specified StorageVersionMigration"),
		...
		mcp.WithObject("spec", mcp.Description("Input parameter: Spec of the storage version migration.")),
		mcp.WithObject("status", mcp.Description("Input parameter: Status of the storage version migration.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    Replacestoragemigrationv1alpha1storageversionmigrationstatusHandler(cfg),
	}
}
